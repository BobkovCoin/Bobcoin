...
function writeStructConstructor(type, args, ctx) {
    ...
    const name = ops_1.ops.typeContsturctor(type.name, args, ctx);
...
function writeExpression(f, ctx) {
    ...
    //
    if (f.kind === 'string') {
        const id = (0, writeConstant_1.writeString)(f.value, ctx);
        ctx.used(id);
        return `${id}()`;
    }
    //
    ...
    if (f.kind === 'op_call') {
        ...
        if (src.kind === 'ref') {
            ...
            // Check struct ABI
            if (t.kind === 'struct') {
                const abi = struct_1.StructFunctions[f.name];
                if (abi) {
        ...
        // Map types
        if (src.kind === 'map') {
            const abf = map_1.MapFunctions[f.name];
            if (!abf) {
