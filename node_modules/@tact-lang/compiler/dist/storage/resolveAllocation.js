...
function resolveAllocations(ctx) {
    ...
    for (const s of types) {
        ...
        // Perform allocation
        const root = (0, allocator_1.allocate)({ ops, reserved: { bits: reserveBits, refs: reserveRefs } });
        const partialRoot = (0, allocator_1.allocate)({ ops: partialOps, reserved: { bits: reserveBits, refs: reserveRefs } });
        // Store allocation
        const allocation = {
            ops,
            root,
    ...
    for (const s of types) {
        if (s.kind === 'contract' && s.init) {
            ...
            // Perform allocation
            const root = (0, allocator_1.allocate)({ ops, reserved: { bits: reserveBits, refs: reserveRefs } }); // Better allocation?
            // Store allocation
            const allocation = {
                ops,
                root,
