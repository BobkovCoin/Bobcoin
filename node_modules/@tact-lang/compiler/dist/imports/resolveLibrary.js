...
function resolveLibrary(args) {
    ...
    if (args.name.startsWith('@stdlib/')) {
        ...
        const libraryPath = (0, parseImportPath_1.parseImportPath)('./' + libraryName + '.tact');
        if (!libraryPath) {
            return { ok: false };
        }
    ...
    const kind = importName.endsWith('.fc') ? 'func' : 'tact';
    if (!importName.endsWith('.tact') && !importName.endsWith('.fc')) {
        importName = importName + '.tact';
    }
    // Resolve import
    const parsedImport = (0, parseImportPath_1.parseImportPath)(importName);
    if (!parsedImport) {
        return { ok: false };
    }
